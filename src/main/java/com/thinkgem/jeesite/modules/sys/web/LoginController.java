...
public class LoginController extends BaseController {
	...
	public String index(HttpServletRequest request, HttpServletResponse response) {
	    ...
	    if (user2.getCount()==0) {//判断是否第一次登陆，如果是则进入密码修改页面
	    	if (user2.getId().equals("1")) {
		        return "forward:"+adminPath+"/school/schoolinfo/form1?id="+user2.getSchoolid();
			}else {
				return "redirect:" + adminPath + "/sys/user/modifyPwdnew";
			}
		}
		// 登录成功后，验证码计算器清零
		...
		if (principal.isMobileLogin()) {
			/*if (request.getParameter("login") != null) {
				System.out.println("--------------------1---------------------"+request.getParameter("login"));
				return renderString(response, principal);
			}*/
			if (principal.getSessionid()!=null&&principal.getSessionid().length()>0) {
