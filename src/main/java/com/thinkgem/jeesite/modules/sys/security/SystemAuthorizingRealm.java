	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		if (user != null) {
			...
			if (Global.TRUE.equals(Global.getConfig("user.multiAccountLogin"))) {
				//获取用户缓存
				Collection<Session> sessions1 = getSystemService().getSessionDao()
						.getActiveSessions(false, new Principal(user,false), UserUtils.getSession());
				if (sessions1.size() > 0) {
					for(Session s: sessions1){
						//用户id
						String id = String.valueOf(s.getAttribute(DefaultSubjectContext.PRINCIPALS_SESSION_KEY));
						if(id!=null && !"".equals(id) && !"null".equals(id)){
							// 如果是登录进来的，则踢出已在线用户
							if (user.getId().equals(id)) {
								getSystemService().getSessionDao().delete(s);
