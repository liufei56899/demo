...
$.extend($.fn, {
    ...
    loadData: function(flag) {
        return this.each(function() {
            ...
            var secOption = $(this).attr('name').trim();
            
            var t2value = $(this).children().text().trim();
            var t3value = $("#current_path").text().trim();

            t3value = t3value.substr(0,t3value.lastIndexOf(">")).trim();
            t3value = t3value.substr(0,t3value.indexOf(":")+1).trim();

            var mainpage = (t3value+">"+secOption+">"+t2value).trim();
            
            if((mainpage.match("主页")!=null)&&(mainpage.match(">当前位置:主页")==null)){
            	mainpage = "当前位置:主页";
            }else if(mainpage.match(">当前位置:主页")!=null){            	
            	mainpage = "当前位置:主页";
            }else{
            	if((mainpage.match("当前位置:")==null)&&(mainpage.match("当前位置:>")==null)){         		
            		if((mainpage.match("当前位置:>")==null)){
            			mainpage = "当前位置:"+(t3value+">"+secOption+">"+t2value).substr(6,(t3value+">"+secOption+">"+t2value).length).trim();
            		}else{
            			mainpage = "当前位置:"+(t3value+">"+secOption+">"+t2value).substr(1,(t3value+">"+secOption+">"+t2value).length).trim();
            		}
            	}else {	
            		if((mainpage.match("当前位置:>")!=null)){
        			   mainpage = "当前位置:"+(t3value+">"+secOption+">"+t2value).substr(6,(t3value+">"+secOption+">"+t2value).length).trim();
        		    }else{
        		    	mainpage = (t3value+">"+secOption+">"+t2value).trim();
        		    }
            	}
            }

            $("#current_path").text("".trim());
            $("#current_path").append(mainpage);
