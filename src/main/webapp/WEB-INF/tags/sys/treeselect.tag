	...
	$("#${id}Button, #${id}Name").click(function(e){
		...
		top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}", "选择${title}", 300, 420, {
			ajaxData:{selectIds: $("#${id}Id").val()},buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
			    ...
			    var _scrollHeight = $(top.document).scrollTop();//获取当前窗口距离页面顶部高度 
                var _windowHeight = $(top.window).height();//获取当前窗口高度 
                var _windowWidth = $(top.window).width();//获取当前窗口宽度 
                var _posiTop = (_windowHeight - 420)/2 - _scrollHeight; 
                var _posiLeft = (_windowWidth - 300)/2; 
			    $(".jbox", top.document).css({"top":_posiTop+"px","left":_posiLeft+"px"});
