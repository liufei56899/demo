...
<body>
	...
	<script type="text/javascript">
	      ...
	      var zYId  = "${xsJbxx.zyId.id}";//专业id
		  var zYLxId = "${xsJbxx.zylxId.id}";//专业类别ID
		$(document).ready(function() {
			var zylx="${xsJbxx.zylxId.id}";
			$("#zylx").select2("destroy");
			$("#zylx").val(zylx);
			$("#zylx").select2();
		    //console.log("${xsJbxx.zyId.id}===="+"${xsJbxx.zyId.zyjc}");
		     isOutSchool();
		     getBanJi("${xsJbxx.zyId.id}");
		     getBgydlx();
		     pinYinShengCheng();
			$("#inputForm").validate({
	...
	function openHistoryRecord(value) {//formXsJbxxHistory
		...
		function zhuanYeInit()
		{
			...
			//-----------------------------------------------------20161121----------------------------------
		       ...
		       //alert(result.html);
		        ...
		        $("#zyid").attr("onchange","getBanJi(this.value);findXueZhiById(this,'${ctx}','xzLab','xzId');");
		        $("#zyid").select2({
				 	formatResult: format,
		...
		function zhuanYeOnClick(zylxVal)
		{
			 ...
			 $.getJSON("${ctx}/zy/zyJbxx/findZysByLxIdForBgyd",{id:zylxVal},function(result){
		        $("#zhuaYeId").html(result.html);
			  	$("#zyid").attr("style","width:192px;");
			  	$("#zyid").attr("onchange","getBanJi(this.value);findXueZhiById(this,'${ctx}','xzLab','xzId');");
	...
	function getBgydlx(){
		...
		if(bgydlx=="0"){
			$("#banji1").removeAttr("disabled");
			$("#zylx").attr("disabled","true");
			$("#zyid").attr("disabled","true");
			$("#xzlab").attr("disabled","true");
			$("#bjmcFont").attr("color","red");
			$("#zymcFont").removeAttr("color");
			$("#zylxFont").removeAttr("color");
		}else if(bgydlx=="1"){
			$("#bjmcFont").attr("color","red");
			$("#zylxFont").attr("color","red");
			$("#zymcFont").attr("color","red");
			$("#zylx").removeAttr("disabled");
			$("#zyid").removeAttr("disabled");
			$("#banji1").removeAttr("disabled");
			$("#xzlab").removeAttr("disabled");
		}else{
			$("#bjmcFont").removeAttr("color");
			$("#zymcFont").removeAttr("color");
			$("#zylxFont").removeAttr("color");
			$("#zylx").attr("disabled","true");
			$("#banji1").attr("disabled","true");
			$("#zyid").attr("disabled","true");
			$("#xzlab").attr("disabled","true");
		}
	}
	...
	function getBanJi(id){
		...
		jQuery.ajax({
					type : 'post',
					url : "${ctx}/xsfb/xsfb/getBanJi?zyid="+id,
					dataType : 'json',
					data : {
						njid : nj
					},
					...
					success : function(data) {
						console.log(data);
					$("#banji1").select2("destroy");
					$("#banji1").empty();
					$("#banji1").append('<option value="">请选择</option>');
						...
						$("#banji1").append(
						"<option value='"+data[i].id+"' "+html+">"+data[i].bjmc+"</option>"
						)
						}
						$("#banji1").select2();
					}
	...
	function tijiao(){
		var bgydlx=$("#bgydlx").val();
		var newBjId=$("#banji1").find("option:selected").val();
		
		if(bgydlx=="0"){
			if(newBjId=="" || newBjId==null){
				alertx("提交时班级名称不能为空！");
			}else if(bjid==newBjId){
				alertx("调班时不能与原班级相同！");
			}else{
				$("#inputFormStu").submit();	
			}
		}else if(bgydlx=="1"){
			var newZyId=$("#zyid").val();
			if(zYId==newZyId){
				alertx("调专业时不能与原专业相同！");
			}else if(newBjId=="" || newBjId==null){
				alertx("提交时班级名称不能为空！");
			}else{
				$("#inputFormStu").submit();
			}
	...
	<form:form id="inputFormStu" modelAttribute="xsJbxx" action="${ctx}/xs/xsJbxx/savexsbgyd" method="post" class="form-horizontal" enctype="multipart/form-data">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>	
		<fieldset>
			<legend>学生信息变更异动类型:
			<form:select path="bgydlx" class="input-xlarge" id="bgydlx" onchange="getBgydlx();" >
		...
		<div class="control-group">
			...
			<div class="lg-form">
				<label class="control-label1">姓名拼音：</label>
					<form:input path="xmpy" htmlEscape="false" readonly="true" maxlength="64" id="xmpy" onblur="xmpyAs();"
						class="input-xlarge" />
			</div>
		</div>
		<div class="control-group">
			<div class="lg-form">
				<label class="control-label1">英文姓名：</label>
					<form:input path="ywxm" htmlEscape="false" readonly="true" maxlength="32" class="input-xlarge"/>
			</div>
			<div class="lg-form">
				<label class="control-label1" >性别：</label>
				<form:select path="xbm" class="input-xlarge " disabled="true" >
						<form:option value="" label=""/>
		...
		<div class="control-group">
			<div class="lg-form">
				<label class="control-label1" >身份证件类型：</label>
					<form:select path="sfzjlxm" class="input-xlarge " id="sfzjlxm" disabled="true">
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				<label class="control-label1 ">身份证件号码</label>
					<form:input path="sfzjh" id="sfzjhmm" htmlEscape="false" maxlength="18"  readonly="true"/>
			</div>
		</div>
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
                         <input name="csrq" type="text" maxlength="20" class="input-medium Wdate" disabled="disabled"
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="lxdh" id="lxdh" readonly="true" onblur="validateDh('lxdh');" htmlEscape="false" maxlength="32" class="input-xlarge " />
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="jkzkm" class="input-xlarge " disabled="true">
						<form:option value="" label=""/>
		...
		<div class="control-group">
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="hyzkm" class="input-xlarge" disabled="true">
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="gjdqm" class="input-xlarge required" disabled="true">
						<form:option value="" label=""/>
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="hkszdqxyxxxdz"  readonly="true" htmlEscape="false" maxlength="64" class="input-xlarge required" onblur="Adress();" id="hkszdqxyxxxdz" />
				</div>
			</div>
			<div class="lg-form">
				<label class="control-label1">家庭现地址：</label>
				<div class="controls1">
					<form:input path="jtxdz" readonly="true" htmlEscape="false" maxlength="128" class="input-xlarge " />
				</div>
			</div>
		</div>
		 <div class="control-group">
			<div class="lg-form">
				<label class="control-label1"><font id="zylxFont">专业类别：</font></label>
				<div class="controls1">
					<form:select path="" id="zylx" class="input-xlarge required" disabled="true" onchange="zhuanYeOnClick(this.value);">
						<form:option value="" label=""/>
		...
		<div class="lg-form">
				<label class="control-label1 "><font id="zymcFont">专业名称：</font></label>
				<div class="controls1" id="zhuaYeId">
					<form:select path="zyId.id" class="input-xlarge required" id="zyid" disabled="true" >
						<option value=""></option>
					</form:select> 
					<span class="help-inline"><font color="red">*</font> </span>
					<div class="controls1">
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="nj.id" id="njid" class="input-xlarge"  disabled="true">
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<label id="xzLab">${fns:getDictLabel(xsJbxx.zyId.xz, 'xzdm', '')}</label>
						<input type="hidden" name="xz" id="xzId" value="${xsJbxx.zyId.xz}" />
				</div>
			</div>
		</div>
		<div class="control-group">
			<div class="lg-form">
					<label class="control-label1"><font id="bjmcFont">班级名称：</font></label>
					<div class="controls1">
					<form:select path="newBjmc"  id="banji1" class="input-xlarge required"  style="width:184px;">
					</form:select>
					<span class="help-inline"><font color="red">*</font> </span>
					</div>
			</div>
		...
		<div class="form-actions">
			...
			<shiro:hasPermission name="xs:xsJbxx:edit">&nbsp;</shiro:hasPermission>
