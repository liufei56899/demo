...
<body>
	<style type="text/css">
		...
		.form-horizontal .lg-form .controls1 .input-xlarge
		{
			width:180px;
		}
		.form-horizontal .lg-form .controls1 .select2-container
		{
			width:192px;
		}
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
		    //console.log("${xsJbxx.zyId.id}===="+"${xsJbxx.zyId.zyjc}");
		     isOutSchool();
		     pinYinShengCheng();
			$("#inputForm").validate({
			...
			chengYuanYiOnBlur();
			chengYuanErOnBlur();
			var zyid = "${xsJbxx.zyId.id}";
			var bjid = "${xsJbxx.bjmc.id}";
			
			initBj(zyid,bjid);
		});
		
		function initBj(zyid,bjid)
		{
		  	//alert(zyid);
			jQuery.ajax({
			        type: "POST",
			        url: "${ctx}/xs/xsJbxx/getBanJiByZyid",
			        data: {bjid:bjid,zyid:zyid},
			        dataType:'json',
			        async:false,
					success: function(result){
						$("#bjDiv").html(result.bjHtml);
						$("#bjID").select2();
				   }
			 });
		  	 ...
		  	 jQuery.ajax({
			        type: "POST",
			        url: "${ctx}/zy/zyJbxx/findZyfxAndZyjc",
			        data: {id:zyid},
			        dataType:'json',
			        async:false,
					success: function(result){
						$("#zyjc").val(result.zyjc);
						$("#zyfx").val(result.zyfxmc);
				   }
			 }); 
		}
	...
	function openHistoryRecord(value) {//formXsJbxxHistory
		...
		function zhuanYeInit()
		{
			...
			//alert(zhuanYeId+"      "+zhuanYeLxId)
			
			var bj = "${xsJbxx.bjmc.id}";
			...
			//-----------------------------------------------------------------
			 jQuery.ajax({
			        type: "POST",
			        url: "${ctx}/xs/xsJbxx/getBan",
			        dataType : 'json',
			        data: {id:zhuanYeLxId,zy:zhuanYeId,bj:bj},
					success: function(data){
						//alert(data.zymc+"    vvvv")
						$("#fang").val(data.zymc);
						$("#banji").val(data.jbxx);
			...
			//-----------------------------------------------------20161121----------------------------------
		       ...
		       //alert(result.html);
		        $("#zyid").attr("style","width:192px;");
		        $("#zyid").attr("onchange","findBj();");
		        $("#zyid").select2({
				 	formatResult: format,
		...
		function onchange(){
		   /*  var isTrue1 = validateDh("jhrlxdh");
			var	isTrue2 = validateSfz();
			var	isTrue3 = validateDh("lxdh");
			var	isTrue4 = validateDh("jtdh");
			var isTrue5 = validateDh("cyylxdh");
			var	isTrue6= validateDh("cyelxdh");
			var	isTrue7 = checkRs(); */
            var isTrue8 = xmpyAs();
            var w=quNumber();
            var a=asNumber();
           ...
           // var d=isOutSchool();
             ...
             alert("d"+d);
             alert("c"+c); */
            return isTrue8&&w&&a&&j&&r&&p&&x&&c&&d;//&&h&&d;
		}
		
		function checkRs()
		{
			var banJiId = $("#bjID").val();
			var syxs =0;
			var ret = true;
			jQuery.ajax({
				type : 'post',
				url : "${ctx}/xsfb/xsfb/getBanJiInfo",
				dataType : 'json',
				data : {
					id : banJiId
				},
				async : false,
				success : function(data) {
					syxs = data.bjxx.remarks;
					//$("#syxs").val(data.bjxx.remarks);
				}
			});
			var bj = "${xsJbxx.bjmc.id}";
			//console.log(banJiId+"****11*******"+bj);
			if(banJiId != bj)
			{
				if(parseInt((syxs+1)) > parseInt(syxs))
				{
					ret = false;
				}
			}
		...
		//姓名转化拼音
		function pinYinShengCheng(){
    	var xm=$("#xm").val();
    	if(xm!=null || xm!=""){
    		$("#xmpy").val(ConvertPinyin(xm));
    	}
   	 }  
		//招生方式
		/**
		...
		*/
		...
		//是否校外教学点学生验证
		/**
		 是否校外教学点学生验证 为是时，则校外教学点为必填项
		*/
		function isOutSchool(){
			var sfxwjxdxs=$("#sfxwjxdxs").val();
			if(sfxwjxdxs =="1")
			{
				$("#xwjxd").addClass("required");
				$("#xwjxd").next().html("<font color='red' id='help8'>*</font>");
			}
			else
			{
				$("#xwjxd").removeClass("required");
				$("#xwjxd").next().children().remove();
			}
		}
		//连招合作类型
		...
		**/
		...
		//==========================生源地行政区域码==================
		function quNumber(){
           var num=$("#sydxzqhm").val();
           var quyu=/^[0-9]{6}$/;	
          
           if(num!=null||num!=""){
                if(!quyu.test(num)){
		
                   $("#help1").html("区域码是6位正整数数字！");
                   return false;
                }
                $("#help1").html("*");
                return true;
 		...
 		//=========================出生地行政区域码==================
 		function asNumber(){
 		   var num=$("#csdxzqhm").val();
 		   var hm=/^[0-9]{6}$/;	
 		   
           if(num!=null||num!=""){
                if(!hm.test(num)){
		
                   $("#help2").html("区域码是6位正整数数字！");
                   return false;
                }
                $("#help2").html("*");
                return true;
 		...
 		//=================================户口所在地地址==============
 		function Adress(){
 		  var ad=$("#hkszdqxyxxxdz").val();
 		 // var sa=/^[u4e00-u9fa5]{0,100}$/;
 		   var sa=/^[\u4e00-\u9fa5]/;
 		...
 		//===============================户口所在地行政区域码=====================
 		function huKou(){
 		  var hk=$("#hkszdxzqhm").val();
 		  var hm=/^[0-9]{6}$/;
 		  if(hk!=null||hk!=""){
 		  if(!hm.test(hk)){
 		      $("#help6").html("区域码是6位正整数数字!");
 		      return false;
 		  }
 		   $("#help6").html("*");
 		       return true; 
 		...
 		function tjButton(){
 			document.getElementById("sub").value="2"; 
 			//alert("2");
			return true;
 		}
...
function previewFile(index) {
		var imgShow="imgShow"+index;
		var pic="pic"+index;
		 var preview = document.querySelector("img[id="+imgShow+"]");
		 var file  = document.querySelector("input[class="+pic+"]").files[0];
	...
	<form:form id="inputForm" modelAttribute="xsJbxx" action="${ctx}/xs/xsJbxx/save" method="post" class="form-horizontal" enctype="multipart/form-data">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>	
		<!-- <div class="control-group" id="divHistory" style="display: none;">
			<label class="control-label1">学生历史记录:</label> 
				<label class="control-label1" style="margin-left: 20px;">
	...
	<div class="cont-group1">
		<div class="left-control-group">
			<div class="left-form">
				...
				<form:input id="xm" path="xm" htmlEscape="false" maxlength="15" readonly="true"
						class="input-xlarge required"/>
					<span class="help-inline"><font color="red">*</font> </span>
			</div>
			<div class="left-form">
				<label class="control-label1">姓名拼音：</label>
					<form:input path="xmpy" htmlEscape="false" maxlength="64" id="xmpy" onblur="xmpyAs();"
						class="input-xlarge required" />
					<span class="help-inline"><font color="red" id="help">*</font></span>
			</div>
			<div class="left-form">
			...
			<div class="left-form">
				<label class="control-label1" ><font color="red">性别：</font></label>
				<form:select path="xbm" class="input-xlarge required" disabled="true" >
		...
		<div class="right-form" style="position: relative;">
				<label class="control-label1" style="line-height: 161px; height: auto;">照片：</label>
				...
				<img src="${xsJbxx.photo}" id="imgShow0" style='width:120px; height:160px;'/>			
					...
					<span class="help-inline"><font color="red">*</font> </span>
					<form:input type="file"  path="file" id="pic" class='pic0' 
					onchange='previewFile(0)' />
					<!-- <input type="button"  style="position: absolute; top:60%; right: 24%;" 
					 class="btn btn-primary butt" value="上传" onclick="getImg()"> -->
			</div>
		...
		<div class="control-group">
			<div class="lg-form">
				<label class="control-label1" ><font color="red">身份证件类型：</font></label>
					<form:select path="sfzjlxm" class="input-xlarge required" id="sfzjlxm" disabled="true">
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				<label class="control-label1 "><font color="red">身份证件号码：</font></label>
					<form:input path="sfzjh" id="sfzjhmm" htmlEscape="false" maxlength="18" disabled="true"
						class="input-xlarge required" onblur="ValidateSfz();" />
					<span class="help-inline"><font color="red" id="line">*</font> </span>
			</div>
		</div>
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
                         <input name="csrq" type="text" maxlength="20" class="input-medium Wdate" disabled="true"
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="mzm" class="input-xlarge" disabled="true">
						<form:option value="" label=""/>
			...
			<%-- <div class="lg-form">
				<label class="control-label1 labWidth" >身份证件类型：</label>
				<div class="controls1">
					<form:select path="sfzjlxm" class="input-xlarge required" id="sfzjlxm" disabled="true">
						<form:option value="" label=""/>
						<form:options items="${fns:getDictList('sfzjlx')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="lxdh" id="lxdh" onblur="validateDh('lxdh');" htmlEscape="false" maxlength="32" class="input-xlarge " />
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="jkzkm" class="input-xlarge required">
						<form:option value="" label=""/>
		...
		<div class="control-group">
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="hyzkm" class="input-xlarge" >
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="gjdqm" class="input-xlarge required">
						<form:option value="" label=""/>
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="hkxz" class="input-xlarge" >
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="xsjzdlx" class="input-xlarge" >
						<form:option value="" label=""/>
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="gatqwm" class="input-xlarge required" >
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="zzmmm" class="input-xlarge">
						<form:option value="" label=""/>
		...
		<div class="control-group">
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="sydxzqhm" htmlEscape="false" maxlength="64" 
					class="input-xlarge required" onblur="quNumber();" id="sydxzqhm"/>
					<span class="help-inline"><font color="red" id="help1">*</font> </span>
				</div>
			</div>
		</div>	
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="csd" htmlEscape="false" maxlength="64"
						class="input-xlarge"/>
				</div>
			</div>
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="csdxzqhm" htmlEscape="false" maxlength="18" class="input-xlarge required"
					onblur="asNumber();" id="csdxzqhm"/>
					<span class="help-inline"><font color="red" id="help2">*</font> </span>
				</div>
			</div>
		</div>
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="jg" htmlEscape="false" maxlength="64"
						class="input-xlarge"/>
				</div>
			</div>
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="jgdxzqhm" htmlEscape="false" maxlength="64" class="input-xlarge required" onblur="jgNumber();" id="jgdxzqhm"/>
					<span class="help-inline"><font color="red" id="help3">*</font> </span>
				</div>
			</div>
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="hkszd" htmlEscape="false" maxlength="64"
						class="input-xlarge"/>
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="hkszdxzqhm" htmlEscape="false" maxlength="64" class="input-xlarge" id="hkszdxzqhm"/>
					<!-- <form:input path="hkszdxzqhm" htmlEscape="false" maxlength="64" class="input-xlarge" onblur="huKou();" id="hkszdxzqhm"/> -->
				</div>
			</div>
			<!--  -->
		</div>
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="hkszdqxyxxxdz" htmlEscape="false" maxlength="64" class="input-xlarge required" onblur="Adress();" id="hkszdqxyxxxdz" />
					<span class="help-inline"><font color="red" id="help4">*</font> </span>
				</div>
			</div>
			<div class="lg-form">
			...
			<%-- <div class="lg-form">
				<label class="control-label1">户口性质：</label>
				<div class="controls1">
					<form:select path="hkxz" class="input-xlarge required" >
						<form:option value="" label=""/>
						<form:options items="${fns:getDictList('hkxz')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
					</form:select>
					<span class="help-inline"><font color="red">*</font> </span>
		...
		<%-- <div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="xz" class="input-xlarge required" >
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				<label class="control-label1 labWidth">民族：</label>
				<div class="controls1">
					<form:select path="mzm" class="input-xlarge required" disabled="true">
						<form:option value="" label=""/>
						<form:options items="${fns:getDictList('nation')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
					</form:select>
					<span class="help-inline"><font color="red">*</font> </span>
		...
		<div class="control-group">
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="jtdh" id="jtdh" onblur="validateDh('jtdh');" htmlEscape="false" maxlength="32" class="input-xlarge " />
					<span class="help-inline"></span>
				</div>
			</div>
		</div>
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="scpcs" htmlEscape="false" maxlength="64" class="input-xlarge required" onblur="Paichusuo()" id="scpcs" />
					<span class="help-inline"><font color="red" id="help5">*</font> </span>
				</div>
			</div>
			<div class="lg-form">
				<label class="control-label1">乘火车区间：</label>
				<div class="controls1">
					<form:input path="chcqj" htmlEscape="false" maxlength="64" class="input-xlarge required" />
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
		</div>
		...
		<!-- <div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="jhrlxdh" id="jhrlxdh" onblur="validateDh('jhrlxdh');" htmlEscape="false" maxlength="32" class="input-xlarge required" />
					<span class="help-inline"><font color="red">*</font></span>
				</div>
			</div>
			<div class="lg-form">
				<label class="control-label1 labWidth" >毕业学校：</label>
				<div class="controls1">
		...
		<div class="control-group">
			<label class="control-label1">备注信息：</label>
			<div class="controls1">
				<form:textarea path="remarks" htmlEscape="false" rows="4" 
					maxlength="255" class="input-xxlarge " style="width:786px;"/>
			</div>
		</div> -->
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="xslym" class="input-xlarge " >
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="zsdx" class="input-xlarge " >
						<form:option value="" label=""/>
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="rxfsm" class="input-xlarge required">
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="sfsqzn" class="input-xlarge required" >
						<form:option value="" label=""/>
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="zssyd" id="zssyd"  htmlEscape="false" maxlength="64" class="input-xlarge " />
				</div>
			</div>		
		</div>
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<input name="rxny" type="text"  maxlength="20" class="input-medium Wdate required"
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="xh" htmlEscape="false" maxlength="18" class="input-xlarge required" onblur="Xxh();" id="xh" />
					<span class="help-inline"><font color="red" id="help7">*</font> </span>
				</div>
			</div>
			<!--  -->
			<!--  -->
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="xslbm" class="input-xlarge required">
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="zsjj" id="zsjj" class="input-xlarge required">
						<form:option value="" label=""/>
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="zyId.zyjc" name="zyjc" htmlEscape="false" maxlength="32" class="input-xlarge " id="zyjc" readonly="true"/>
				</div>
			</div>
			<div class="lg-form">
				<label class="control-label1">学习形式：</label>
				<div class="controls1">
					<form:select path="xxxsm" class="input-xlarge required" >
						<form:option value="" label=""/>
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="xsnj" htmlEscape="false" maxlength="64" class="input-xlarge required"  id="nj" readonly="true" />
					<%-- <form:select path="xsnj" class="input-xlarge" disabled="true" >
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="xz" class="input-xlarge " disabled="true">
						<form:option value="" label=""/>
						<form:options items="${fns:getDictList('xzdm')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
					</form:select>
					<span class="help-inline"><font color="red">*</font> </span>
		...
		<div class="control-group">
			...
			<div class="lg-form">
				...
				<div class="controls1">
					...
					<form:select path="fdpyfs" id="fdpyfs" class="input-xlarge" >
						<form:option value="" label=""/>
			...
			<div class="lg-form">
					...
					<div class="controls1">
					<input class="input-xlarge required" id="banji" readonly="readonly" type="text"/>
					<span class="help-inline"><font color="red">*</font></span>
					</div>
			</div>
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="sfxwjxdxs" id="sfxwjxdxs" class="input-xlarge" onchange="isOutSchool();">
						<form:option  value="" label="" />
						<form:options items="${fns:getDictList('sfdm')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
					</form:select>
				</div>
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="jdfsm" class="input-xlarge required" >
						<form:option value="" label=""/>
		...
		<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="zsfs" id="zsfs" class="input-xlarge" onchange="zhaoShengFangShi();">
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="zkzh" id="zkzh" onblur="zhaoShengFangShi();" htmlEscape="false" maxlength="32" class="input-xlarge required " />
					<span class="help-inline"><font color="red">*</font></span>
				</div>
			</div>
		</div>
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="ksh" id="ksh" onblur="zhaoShengFangShi();" htmlEscape="false" maxlength="32" class="input-xlarge "/>
					<span class="help-inline"></span>
				</div>
			</div>
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="kszf" id="kszf" onblur="zhaoShengFangShi();" htmlEscape="false" maxlength="10" class="input-xlarge " />
					<span class="help-inline"></span>
				</div>
			</div>
			<!-- <div class="lg-form">
				<label class="control-label1 labWidth" >考生特长：</label>
				<div class="controls1">
		...
		<!-- <div class="control-group">
			...
			<div class="lg-form">
				<label class="control-label1 labWidth">体检结论：</label>
				<div class="controls1">
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="lzhzlx" id="lzhzlx" class="input-xlarge"  onchange="lianZhaoHeZuoLeiXing();">
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="lzhzxxmc" id="lzhzxxmc" htmlEscape="false" maxlength="32" class="input-xlarge " />
					<span class="help-inline"> </span>
				</div>
			</div>
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1"> 
					<form:select path="lzhzbxfs" id="lzhzbxfs" class="input-xlarge required" onblur="lianZhaoHeZuoLeiXing();" >
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="lzhzxxdm" id="lzhzxxdm" htmlEscape="false" onblur="lianZhaoHeZuoLeiXing();" maxlength="32" class="input-xlarge required" />
					<span class="help-inline"> </span>
				</div>
			</div>
		
			<%-- <div class="lg-form">
				<label class="control-label1 labWidth" >联招合作学校办学类型：</label>
				<div class="controls1"> 
		...
		<div class="control-group">
			 <div class="lg-form">
				...
				<div class="controls1">
					<form:input path="xwjxd" htmlEscape="false" maxlength="32" class="input-xlarge " onchange="Xwjxd();" id="xwjxd"/>
					<span class="help-inline"><font color="red" id="help8"></font> </span>
				</div>
			</div>
			 <div class="lg-form">
				<input type="hidden" width="200">
			</div>
		</div>
		...
		<%-- <div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:select path="zylxId.id" class="input-xlarge required" disabled="true" onchange="zhuanYeOnClick(this.value);">
						<form:option value="" label=""/>
			...
			<div class="lg-form">
				<label class="control-label1 labWidth">专业名称：</label>
				<div class="controls1" id="zhuaYeId">
				
					<form:select path="zyId.id" class="input-xlarge required" id="fang" disabled="true">
						<option value=""></option>
					</form:select> 
					
					<span class="help-inline"><font color="red">*</font> </span>
					<div class="controls1">
					<input class="input-xlarge required" id="fang" readonly="readonly" value="" type="text"/>
				    </div>
			</div>
		</div>
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls1">
					<form:input path="zyId.zyfxmc" name="zyfx" htmlEscape="false" maxlength="32" class="input-xlarge " id="zyfx" readonly="true"/>
				</div>
			</div>
		</div> --%>
		
		<input type="hidden" name="sub" id="sub" value="4">
		
		<%-- <c:if test="${not empty xsJbxx.id}"> --%>
			<%-- <div class="control-group">
		  <div class="lg-form">
			<label class="control-label1">审核人：</label>
			<div class="controls1">
			  <input type="text" name="userName1" id="approveBy.name" disabled="disabled" class="input-xlarge "
			  	  style="width:180px;" value="${xsJbxx.approveBy.name }"  />
			</div>
		  </div>
		   <div class="lg-form">
			     <label class="control-label1">审核时间：</label>
			     <div class="controls1 labWidth">
				     <input type="text" disabled="disabled" maxlength="64" class="input-xlarge Wdate required" style="width:180px;"
		...
		<div class="form-actions">
			<shiro:hasPermission name="xs:xsJbxx:edit"><input id="btnSubmitSave" onclick="return saveBefferCheck()" class="btn btn-primary"  value="保存" type="submit" />&nbsp;</shiro:hasPermission>
			<!-- <input id="btnCancel" class="btn" type="button" value="返 回" onclick="history.go(-1)"/> -->
		<!-- <shiro:hasPermission name="xs:xsJbxx:edit"><input id="but1"  class="btn btn-primary" type="submit" onclick="return tjButton()" value="提交" />&nbsp;</shiro:hasPermission>	onfocus="validateSub();"	 -->
