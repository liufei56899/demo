...
<script type="text/javascript">
		$(document).ready(function() {
			...
			var id1="${zyJbxx.id}";
			if(id1!=null&&id1!=""){
				var zymlbb="${zyJbxx.zymlbb}";
				if(zymlbb!=0){
					//通过 专业版本 加载 专业类别 选框
					loadZylx("${zyJbxx.zymlbb}");
					//设置专业类别的选中值 
					var lxmc = "${zyJbxx.zylx.lxmc}";
					var zylx_title = document.getElementById("zylxid");
					var zylx_title_value="";
					for ( var i = 0; i < zylx_title.options.length; i++) {
						if (zylx_title.options[i].innerHTML == lxmc) {
							zylx_title_value=zylx_title.options[i].value;
							$("#zylxid").select2("destroy");
							$("#zylxid").find(
									"option[value='" + zylx_title_value
											+ "']").attr("selected", true);
							$("#zylxid").select2();
							break;
						}
					...
					if(zylx_title_value!=""){
						/* if(zylx_title_value==19){
							$("#zy_Id").empty();
				     		$("#zy_Id").append("<input value='${zyJbxx.zymc}' name='zymc' id='zymc1' htmlEscape='false' maxlength='64' type='text' class='input-xlarge required'/>"
				     		+"<span class='help-inline'><font color='red'>*</font> </span>");
							$("#zyfx_Id").empty();
				     		$("#zyfx_Id").append("<input value='${zyJbxx.zyfxmc}' name='zyfxmc' htmlEscape='false' maxlength='64' type='text' class='input-xlarge required' />"
							+"<span class='help-inline'><font color='red'>*</font> </span>");
						}else{ */
							//通过选中的专业类别 加载 专业名称 选框
							loadZy(zylx_title_value);
							//设置专业名称的选中值
							var zymc="${zyJbxx.zymc}";
							var zymc1_title = document.getElementById("zymc1");
							var zymc1_title_value="";
							for ( var i = 0; i < zymc1_title.options.length; i++) {
								if (zymc1_title.options[i].innerHTML == zymc) {
									zymc1_title_value=zymc1_title.options[i].value;
									$("#zymc1").select2("destroy");
									$("#zymc1").find(
											"option[value='" + zymc1_title_value
													+ "']").attr("selected", true);
									$("#zymc1").select2();
									break;
								}
							}
							if(zymc1_title_value!=""){
								//通过选中的专业名称 加载 专业技能方向 选框
								loadZyjnfx(zymc1_title_value);
								//设置专业技能方向的选中值
								var zyfxmc="${zyJbxx.zyfxmc}";
								var zyfxmc_title = document.getElementById("zyfxmc");
								var zyfxmc_title_value="";
								for ( var i = 0; i < zyfxmc_title.options.length; i++) {
									if (zyfxmc_title.options[i].innerHTML == zyfxmc) {
										zyfxmc_title_value=zyfxmc_title.options[i].value;
										$("#zyfxmc").select2("destroy");
										$("#zyfxmc").find(
												"option[value='" + zyfxmc_title_value
														+ "']").attr("selected", true);
										$("#zyfxmc").select2();
										break;
									}
				    ...
				    +"<span class='help-inline'><font color='red'>*</font> </span>");
					$("#zyfx_Id").empty();
				    $("#zyfx_Id").append("<input value='${zyJbxx.zyfxmc}' name='zyfxmc' htmlEscape='false' maxlength='64' type='text' class='input-xlarge' />"
					+"<span class='help-inline'><font color='red'></font> </span>");
					$("#bxzybh").empty();
					$("#bxzybh").append("<input value='${zyJbxx.bxzybh}' name='bxzybh' htmlEscape='false' maxlength='64' class='input-xlarge'/>"
					+"<span class='help-inline'><font color='red'></font> </span>")
					$("#zyjc").empty();
					$("#zyjc").append("<input value='${zyJbxx.zyjc}' name='zyjc' htmlEscape='false' maxlength='64' class='input-xlarge'/>"
					+"<span class='help-inline'><font color='red'></font> </span>")
		     ...
		     if(info!=null&&info!=""){
		     	...
		     	if(info==19){
					...
					$("#zyfx_Id").empty();
		     		...
		     		$("#zy_Id").empty();
		     		$("#zy_Id").append("<select name='zymc' id='zymc1' class='input-xlarge required' onchange='loadZyjnfx(this.value)'>"
						+"<option value=''></option>"
					+"</select>"
		     		+"<span class='help-inline'><font color='red'>*</font> </span>");
					$("#zyfx_Id").empty();
		     		$("#zyfx_Id").append("<select name='zyfxmc' id='zyfxmc' class='input-xlarge'>"
						+"<option value=''></option>"
					+"</select>");
					$.ajax({
				        type: "POST",
		    	 ...
		    	 $.ajax({
						...
						success: function(result){
							if(result!=null&&result!=""){
								  ...
								  $("#zyfxmc").select2("destroy");
								  $("#zyfxmc").append("<option value='' selected='true'></option>");
								  var results=result.professionalEmphasis.split("|");
	...
	<form:form id="inputForm" modelAttribute="zyJbxx" action="${ctx}/zy/zyJbxx/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			<div class="lg-form">
				...
				<div class="controls" id="bxzybh">
					<form:input path="bxzybh" htmlEscape="false" maxlength="64" class="input-xlarge required" 
					/>
						<!-- <form:input path="bxzybh" id="zyjss" htmlEscape="false" maxlength="64" class="input-xlarge required" 
						/> -->
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
