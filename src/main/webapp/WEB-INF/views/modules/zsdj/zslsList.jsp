...
<head>
	...
	<script type="text/javascript">
		...
		function add()
        {
        	openNewDialog("${ctx}/zsdj/zsdj/listzs","招生老师登记",1100,480);
        }
		...
		function editJh()
		{
			...
			if(count == 1)
			{
				if(ids!=null && ids!="")
				{
				   ...
				   openNewDialog("${ctx}/zsdj/zsdj/listzs?id="+ids,"修改招生登记信息",1100,480);
				}
			}
			else if(count ==0)
			{
		...
		function deleteJh()
		{
			...
			}else
			{
				...
				confirmx('确认要删除招生登记信息吗？', "${ctx}/zsdj/zsdj/delete?ids="+ids+"&index="+3);
			}
		}
		//批量导入学生信息
		function piLiangDaoRu()
        {
        	openNewDialog("${ctx}/zsdj/zsdj/listmmDRTest/1","招生老师批量导入界面",1100,340);
        }
...
<body>
<div id='dialogzslsDiv' style='width:820px;'></div>
	<form:form id="searchForm" modelAttribute="zsdj" action="${ctx}/zsdj/zsdj/zslslist" method="post" class="breadcrumb form-search">
