...
<head>
	...
	function piliangSubmit() {
		...
		if (count == 0) {
		    ...
		    openDialog("${ctx}/jhfjgr/jhFjGr/formsh?id=" + ids,"审核个人任务分解", 850, 550);
		}
		else {
		 if (ids != null && ids != "") {
				ids = ids.substring(0, ids.length - 1);
		...
		$("#dialogDiv2").dialog({
				    ...
				    width : "400",
		            height : "300",
				    content :"<div style=\"text-align: left;margin-top: 10px;\"><p>对以下个人任务分解进行审核:"+
				             "<span style=\"color:red;\">(注意:审核通过的任务不允许修改!)</span></p>"+
				               jsnames+"</div>",
					buttons : [ {
						text : "审核通过",
						handler : function() {
						    $.getJSON("${ctx}/jhfjgr/jhFjGr/moreShenHe",{ids:ids,shStatu:"1"},function(result){
			  					if(result == "1"){
			  						alertx("分解任务审核'通过'成功!");
			  					      //刷新列表
			                         $("#searchForm").attr("action","${ctx}/jhfjgr/jhFjGr/shList");
						             $("#searchForm").submit();
						             closeDialog();
			  					 }else if(result == "0"){
			  					     alertx("分解任务审核'通过'失败!");
						...
						text : "审核不通过",
						handler : function() {
							$.getJSON("${ctx}/jhfjgr/jhFjGr/moreShenHe",{ids:ids,shStatu:"2"},function(result){
			  					if(result == "2"){
...
<body>
	...
	<form:form id="searchForm" modelAttribute="jhFjGr" action="${ctx}/jhfjgr/jhFjGr/shList/" method="post" class="breadcrumb form-search">
		...
		<table class="ul-form">
			...
			<th style="text-align: right;">教师名称</th>
			<td>	
		...
		<c:forEach items="${page.list}" var="jhFjGr">
			<tr>
				<td>
						...
						<input type="hidden" name="name" value="${jhFjGr.jsmc}" />
						<input type="hidden" name="status" value="${jhFjGr.bcStatus}" />  			
				</td>
				<td>
					${jhFjGr.zsjh.jhmc}
				</td>
